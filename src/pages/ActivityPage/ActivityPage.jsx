...
function ActivityPage() {
  ...
  const fetchActivityData = async () => {
    ...
    try {
      ...
      if (data.location) {
        ...
        fetchNearbyActivities(data.location.latitude, data.location.longitude); // Fetch nearby activities
      }
  ...
  const fetchNearbyActivities = async (latitude, longitude) => {
    try {
      const response = await fetch(`http://localhost:8080/ai/nearby?latitude=${latitude}&longitude=${longitude}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      const data = await response.json();
      // The API returns a list of activities, so we can directly set it.
      // We'll then slice this data for restaurants and attractions in the render.
      setNearbyAttractions(data || []);
    } catch (err) {
      console.error("Error fetching nearby activities:", err);
      // Optionally set an error state for nearby activities
    }
  };
  ...
  useEffect(() => {
    if (activityData && activityData.location && mapRef.current) {
      ...
      const map = L.map(mapRef.current).setView([parseFloat(activityData.location.latitude), parseFloat(activityData.location.longitude)], 13);
      ...
      L.marker([parseFloat(activityData.location.latitude), parseFloat(activityData.location.longitude)], { icon: customIcon }).addTo(map)
  ...
  // Slice nearbyAttractions for restaurants and attractions
  const nearbyRestaurants = nearbyAttractions.slice(0, 5);
  const nearbyPlacesAttractions = nearbyAttractions.slice(5, 10); // Renamed to avoid confusion with component
        ...
        <div className="activity-container">
          <div className='Search-tips-Container'>
            ...
            <Nav variant="underline" defaultActiveKey="City" className="activity-tabs">
              <Nav.Item>
                <Nav.Link eventKey="City">{activityData.name}</Nav.Link>
              </Nav.Item>
              <Nav.Item>
